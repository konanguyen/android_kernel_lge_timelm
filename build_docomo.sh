...
echo "Set DEFCONFIG"
echo 
make CC=clang AR=llvm-ar NM=llvm-nm OBJCOPY=${CROSS_COMPILE}objcopy OBJDUMP=llvm-objdump STRIP=llvm-strip O=out ARCH=arm64 SUBARCH=arm64 vendor/timelm_dcm_jp-perf_defconfig -j$(nproc --all)
